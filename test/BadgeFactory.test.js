...
describe("BadgeFactory", function () {
  ...
  describe("createTemplate()", async function () {
    ...
    it("should emit the appropriate event when a template is created", async function () {
      ...
      expectEvent(receipt, "NewTemplate", {
    ...
    it("random address should not be able to create a new template", async () => {
      await expectRevert(
        factory.createTemplate(template_name, template_description, template_image, { from: random }),
        "BadgeFactory: caller is not a template owner",
      )
    })
  })
  ...
  describe("updateTemplate()", async function () {
    ...
    it("should emit the appropriate event when a template is updated", async function () {
      const receipt = await factory.updateTemplate(templateId, template_name2, template_description2, template_image2, {
        from: owner,
      })
      expectEvent(receipt, "TemplateUpdated", {
        templateId: templateId,
        name: template_name2,
        description: template_description2,
        image: template_image2,
      })
    })

    it("random address should not be able to update a template", async function () {
      await expectRevert(
        factory.updateTemplate(templateId, template_name2, template_description2, template_image2, { from: random }),
        "BadgeFactory: caller is not a template owner",
      )
    })
  })
...
*/
    ...
    it("should revert when templeteId does not exist", async () => {
      await expectRevert(
        factory.activateBadge(proof, offchainId + 1, tokenURI, { from: redeemer }),
        "BadgeFactory: no template with that id",
      )
    })

    it("random address should not be able to activate a new badge", async () => {
      await expectRevert(
        factory.activateBadge(proof, offchainId, tokenURI, { from: random }),
    ...
    it("redeemer should not be able to activate the same badge twice", async () => {
      await factory.activateBadge(proof, offchainId, tokenURI, { from: redeemer })
      await expectRevert(
        factory.activateBadge(proof, offchainId, tokenURI, { from: redeemer }),
  ...
  describe("ERC721 override _transfer()", async function () {
    ...
    it("check transferFrom() for revert", async function () {
      const tokenId = await factory.tokenOfOwnerByIndex(usr, index1, { from: random })
      await expectRevert(
        factory.transferFrom(usr, random, tokenId, { from: usr }),
        "BadgeFactory: badge transfer disabled",
      )
    })
    ...
    it("check safeTransferFrom() for revert", async function () {
      const tokenId = await factory.tokenOfOwnerByIndex(usr, index1, { from: random })
      await expectRevert(
        factory.safeTransferFrom(usr, random, tokenId, { from: usr }),
        "BadgeFactory: badge transfer disabled",
      )
    })
  })
